...
// runPolicy is `hawser policy`: local admission control for the docker API
// (#120).
func runPolicy(args []string) int {
	...
	fmt.Fprintf(os.Stderr, "hawser policy: unknown subcommand %q (show|check|test)\n", args[0])
	return exitUsage
}

func policyUsage() {
	fmt.Fprintf(os.Stderr, `usage: hawser policy show|check|test
...
func runPolicyShow(args []string) int {
	...
	if rules.Empty() {
		...
		fmt.Println("  (write the file to add some; `hawser policy --help` lists the keys)")
		return exitOK
	}
...
func runPolicyTest(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser policy test [--rules <file>] [--json] <create-body.json>|-
...
way to get a real one is `+"`hawser audit tail`"+` on a machine with the audit log on,
or hand-write the fields you care about:

  echo '{"Image":"ubuntu","HostConfig":{"Privileged":true}}' | hawser policy test -
