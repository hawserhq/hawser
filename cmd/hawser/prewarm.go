...
import (
	...
	"github.com/zcsizmadia/hawser/internal/prewarm"
)

// runPrewarm is `hawser prewarm <images.txt>` (#149): pull a pinned image list
...
// local engine, or a `hawser remote`), so they inherit that engine's proxy and
...
func runPrewarm(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser prewarm [--concurrency <n>] [--json] <images.txt>
...
`, exitError, exitUsage)
	...
	if len(refs) == 0 {
		fmt.Fprintf(os.Stderr, "hawser: %s lists no images\n", rest[0])
		return exitUsage
	}
