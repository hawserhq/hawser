...
// gatherStats reads the statistics for `hawser status --stats` (#179).
//
...
func printStats(s statsJSON) {
	...
	if s.Bridge != nil {
		...
		case "socat":
			fmt.Println("  transport         socat — the slow path, pinned by HAWSER_NO_VSOCK")
		case "":
	...
	if e := s.Engine; e != nil {
		...
		if e.ReclaimableBytes > 0 {
			fmt.Printf("  reclaimable       %s  (`hawser prune`)\n", humanBytes(int64(e.ReclaimableBytes)))
		}
	}
	if d := s.Disk; d != nil && d.SizeOnDiskBytes > 0 {
		...
		if d.ReclaimableBytes > 0 {
			fmt.Printf("  reclaimable       ~%s  (`hawser compact`)\n", humanBytes(int64(d.ReclaimableBytes)))
		}
	...
	if v := s.VM; v != nil && v.MemTotalBytes > 0 {
		...
		if v.ConfiguredMemory == "" && v.ConfiguredProcessors == "" {
			fmt.Println("                    no ~/.wslconfig sizing; WSL's defaults apply (`hawser wsl-config`)")
		}
	}
