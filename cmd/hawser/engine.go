...
func runEngine(args []string) int {
	...
	case "-h", "--help", "help":
		...
		return exitOK
	default:
		fmt.Fprintf(os.Stderr, "hawser engine: unknown subcommand %q\n\n", args[0])
		engineUsage(os.Stderr)
		return exitUsage
	}
}
...
func engineUsage(w *os.File) {
	fmt.Fprintf(w, `usage: hawser engine list|upgrade|rollback
...
func engineRef(version, rootfsURL string) string {
	...
	if rest, ok := strings.CutPrefix(base, "hawser-rootfs-"); ok && rest != "" {
		return rest
	}
	return version
}
...
func runEngineList(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "usage: hawser engine list [--json]\n\nflags:\n")
		fs.PrintDefaults()
	}
	...
	if previous != "" {
		fmt.Printf("rollback target: %s (`hawser engine rollback`)\n", previous)
	}
	return exitOK
}
...
func runEngineUpgrade(args []string, rollback bool) int {
	...
	fs.Usage = func() {
		if rollback {
			fmt.Fprintf(os.Stderr, `usage: hawser engine rollback [--dry-run] [--json]

Restores the engine installed before the last `+"`hawser engine upgrade`"+`, from its
...
`)
			...
			fmt.Fprintf(os.Stderr, `usage: hawser engine upgrade [--to <version>] [--dry-run] [--json]

Moves the engine to another version from this build's manifest (`+"`hawser engine list`"+`),
...
is reported.
  ...
  hawser engine upgrade --dry-run        # what it would do
  hawser engine upgrade --to 29.7.3      # a specific engine
  hawser engine rollback                 # back to the previous one
...
`)
	...
	if (*rootfsURL == "") != (*rootfsSHA == "") {
		fmt.Fprintln(os.Stderr, "hawser: --rootfs-url and --rootfs-sha256 go together")
		return exitUsage
	}
	if rollback && (*to != "" || *rootfsURL != "") {
		fmt.Fprintln(os.Stderr, "hawser: rollback takes its target from the install manifest, not --to")
		return exitUsage
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hawser: reading the install manifest: %v\n", err)
		return exitError
	}
	...
	if !*dryRun && err == nil {
		...
		if serr := p.SaveManifest(opts, im); serr != nil {
			fmt.Fprintf(os.Stderr, "hawser: the engine was upgraded but recording it failed: %v\n", serr)
			return exitError
		}
	}
...
func printEngineReport(rep engineupgrade.Report, rollback bool) {
	...
	if !rollback {
		fmt.Printf("  `hawser engine rollback` returns to %s\n", orNone(rep.From))
	}
}
