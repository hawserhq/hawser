...
// runCompact is `hawser compact`: give back the disk space the engine's VHDX is
...
func runCompact(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser compact [--no-trim] [--restart] [--dry-run] [--wait <dur>] [--json]
...
`, "`--restart`", exitError, exitUsage, exitNotFound, exitHeld)
	...
	if _, err := os.Stat(diskPath); err != nil {
		fmt.Fprintf(os.Stderr, "hawser: no virtual disk at %s: %v\n", diskPath, err)
		return exitNotFound
	}
...
func reportCompactError(err error, rep compact.Report, asJSON, restart bool) int {
	...
	case errors.As(err, &held):
		if !asJSON {
			fmt.Fprintf(os.Stderr, "hawser: %s is still open in %s\n", filepath.Base(rep.Path), joinList(held.Holders))
			...
			fmt.Fprintln(os.Stderr, "  Hawser will not stop distros it does not own.")
		}
		return exitHeld
	...
	if !asJSON {
		...
		if rep.Trimmed && rep.ReclaimedBytes == 0 && !restart {
			fmt.Fprintln(os.Stderr, "  The engine is stopped; `hawser start` brings it back.")
		}
	}
	return exitError
}
...
func printCompactReport(rep compact.Report) {
	if rep.DryRun {
		...
		if len(rep.Holders) > 0 {
			...
			fmt.Println("any distro does. Stop them first; Hawser will not stop distros it does not own.")
		}
		return
	}
	...
	if !rep.Restarted {
		fmt.Println("  the engine is stopped; `hawser start` brings it back")
	}
}
