...
// runRelocate is `hawser relocate`: move the engine's data directory, and
...
func runRelocate(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser relocate <new-directory> [--restart] [--dry-run] [--keep-archive] [--json]
...
`, "`--keep-archive`", exitError, exitUsage, exitNotFound, exitNoSpace)
	...
	// invocation like `hawser relocate D:\hawser --dry-run` would otherwise
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hawser: %s: %v\n", target, err)
		return exitUsage
	}
