...
func runRemote(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser remote                                   list remotes
...
func remoteAdd(ctx context.Context, rm *remote.Manager, dc *dockerctx.Manager, name, host, certs string) int {
	if host == "" || certs == "" {
		fmt.Fprintln(os.Stderr, "hawser: remote add needs --host tcp://<host>:<port> and --certs <dir>")
		return exitUsage
	}
	...
	if err := dc.CreateTLS(ctx, remote.ContextName(name), "Hawser remote engine "+name, host, ca, cert, key); err != nil {
		...
		if errors.As(err, &noCLI) {
			fmt.Fprintf(os.Stderr, "hawser: remote %q registered, but no docker CLI to create its context (%v).\n"+
				"Point docker at it with DOCKER_HOST=%s DOCKER_TLS_VERIFY=1 DOCKER_CERT_PATH=%s\n",
				name, err, host, info.Dir)
			return exitOK
		}
		fmt.Fprintf(os.Stderr, "hawser: creating docker context: %v\n", err)
		return exitError
	}

	fmt.Printf("registered remote %q -> %s (client cert valid until %s)\n\n"+
		"  hawser remote test %s     check it answers\n"+
		"  hawser remote use %s      make it docker's default (`hawser remote use local` to go back)\n",
...
func remoteUse(ctx context.Context, rm *remote.Manager, dc *dockerctx.Manager, name string) int {
	if name == "local" {
		...
		fmt.Println("docker now targets the local Hawser engine (context hawser)")
		return exitOK
	}
...
// value: "local" for hawser, a remote's name for hawser-<name>, "" otherwise.
...
func remoteList(ctx context.Context, rm *remote.Manager, dc *dockerctx.Manager, asJSON bool) int {
	...
	if len(infos) == 0 {
		fmt.Println("no remotes; `hawser remote --host tcp://<host>:2376 --certs <dir> add <name>` registers one")
		return exitOK
	}
...
func remoteRemove(ctx context.Context, rm *remote.Manager, dc *dockerctx.Manager, name string) int {
	if !rm.Exists(name) {
		fmt.Fprintf(os.Stderr, "hawser: no such remote %q\n", name)
		return exitNotFound
	}
	...
	if err := dc.RemoveNamed(ctx, remote.ContextName(name), dockerctx.Name); err != nil {
		var noCLI *dockerctx.ErrNoDockerCLI
		if !errors.As(err, &noCLI) {
			fmt.Fprintf(os.Stderr, "hawser: removing docker context: %v\n", err)
			return exitError
		}
	}
...
func remoteTest(ctx context.Context, rm *remote.Manager, name string, asJSON bool) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hawser: remote %q at %s did not answer: %v\n%s\n", name, info.Host, err, text)
		return exitError
	}
	if asJSON {
