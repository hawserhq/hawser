...
func runServe(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser serve --tcp <addr>
...
`, exitError, exitUsage, exitNotFound)
	...
	if *addr == "" {
		fmt.Fprintln(os.Stderr, "hawser: --tcp <addr> is required (e.g. --tcp 0.0.0.0:2376)")
		return exitUsage
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "hawser: listening on %s: %v\n", *addr, err)
		return exitError
	}
	defer ln.Close()
...
func runServeCert(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser serve cert [--host <name-or-ip>] [--client <name>]

Mints the mutual-TLS material for `+"`hawser serve --tcp`"+`: a CA (reused if it
...
Serve it:   hawser serve --tcp 0.0.0.0:2376
...
func serverTLSConfig(dir string) (*tls.Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading server certificate (run `hawser serve cert` first): %w", err)
	}
	caPEM, err := os.ReadFile(filepath.Join(dir, "ca.pem"))
	if err != nil {
		return nil, fmt.Errorf("loading CA (run `hawser serve cert` first): %w", err)
	}
