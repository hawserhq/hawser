...
// Hawser's own settings into the global ~/.wslconfig, with consent (#148).
//
// The two-step shape is deliberate. `hawser config set wsl.memory 4GB` records
// an intention in Hawser's state; this command is the only thing that touches
...
func runWSLConfig(args []string) int {
	...
	case "-h", "--help", "help":
		...
		return exitOK
	default:
		fmt.Fprintf(os.Stderr, "hawser wsl-config: unknown subcommand %q\n\n", args[0])
...
func wslConfigUsage(w *os.File) {
	fmt.Fprintf(w, `usage: hawser wsl-config [show|apply] [--yes] [--json]
...
// desiredFromConfig maps Hawser's wsl.* settings onto .wslconfig key names,
...
func desiredFromConfig(stateDir string) (map[string]string, error) {
	...
	for hawserKey, wslKey := range config.WSLKeys {
		v, err := config.Get(stateDir, hawserKey)
		if err != nil {
			return nil, err
		}
...
func runWSLConfigShow(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "usage: hawser wsl-config show [--json]\n\nflags:\n")
		fs.PrintDefaults()
	}
	...
	if len(plan) == 0 {
		if len(desired) > 0 {
			fmt.Println("\nnothing pending: ~/.wslconfig already matches Hawser's settings")
		} else {
			fmt.Println("\nnothing requested: `hawser config set wsl.memory 4GB` records an intention")
		}
		return exitOK
	}
	fmt.Println("\npending changes (`hawser wsl-config apply` writes them):")
	printWSLPlan(plan)
	return exitOK
}
...
func runWSLConfigApply(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "usage: hawser wsl-config apply [--yes] [--json]\n\nflags:\n")
		fs.PrintDefaults()
	}
	if err := fs.Parse(args); err != nil {
		return exitUsage
	...
	if len(plan) == 0 {
		...
		if len(desired) == 0 {
			fmt.Println("nothing requested; `hawser config set wsl.memory 4GB` records an intention")
		} else {
			fmt.Printf("%s already matches Hawser's settings; nothing to do\n", path)
		}
		return exitOK
	}
	...
	if !*yes {
		if *asJSON {
			fmt.Fprintln(os.Stderr, "hawser: --json needs --yes (there is no way to ask a question in JSON)")
			return exitUsage
		}
	...
	fmt.Println("  `hawser stop` does not do that (it stops only Hawser's distro); a reboot,")
