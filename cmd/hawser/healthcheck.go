...
// runner warm-up scripts, orchestrators and setup-hawser. Ready means a docker
...
// itself: --wait polls while a supervisor brings the engine up (`hawser start`
...
func runHealthcheck(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: hawser healthcheck [--wait <duration>] [--json]
...
`, exitError, exitUsage, exitNotFound)
	...
	if !ok {
		return healthcheckReport(healthcheckJSON{
			Supervisor: "stopped", Engine: "stopped", Reason: "not installed; run `hawser install`",
		}, *asJSON, exitNotFound)
	}
	opts.Distro = distro
...
// probeHealth reads the same facts `hawser status` does and applies the one
// readiness rule.
...
func readiness(supervisorRunning bool, engine string) (ready bool, reason string) {
	...
	case !supervisorRunning:
		return false, "supervisor not running; run `hawser start`"
	case engine == "stopped":
		return false, "engine stopped; run `hawser start`"
	case engine == "idle":
