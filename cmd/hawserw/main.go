// Command hawserw is the windowless launcher and watchdog for the supervisor —
// the javaw convention, plus a restart policy.
//
// It exists first because hawser.exe is a console binary, and anything that
// starts a console binary at logon (a Run key, an interactive scheduled task)
// flashes a console window at the user. hawserw is built for the GUI subsystem
...
// runs `hawser start`. A Go fatal runtime error (hawser#166) cannot be
...
//
// Set HAWSER_NO_WATCHDOG=1 to go back to spawn-and-exit.
package main
...
func main() {
	...
	if os.Getenv("HAWSER_NO_WATCHDOG") == "1" {
		cmd := supervisorCmd(hawser, args)
	...
	os.Exit(watch(hawser, args, stateDir(os.Args[1:])))
}

// supervisorCmd builds the hidden, console-free child process.
func supervisorCmd(hawser string, args []string) *exec.Cmd {
	cmd := exec.Command(hawser, args...)
...
func watch(hawser string, args []string, stateDir string) int {
	...
	// hawserw has no console, so anything the supervisor writes to stderr —
