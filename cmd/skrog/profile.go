...
func runProfile(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: skrog profile                    list profiles (* = active)
...
func listProfiles(m *profile.Manager, asJSON bool) int {
	...
	if len(names) == 0 {
		fmt.Println("no profiles; `skrog profile create <name>` saves the current settings as one")
		return exitOK
	}
	for _, n := range names {
		marker := "  "
...
func showProfile(m *profile.Manager, name string, asJSON bool) int {
	...
	if asJSON {
		// Same document as the YAML: skrogfile.File carries matching json tags.
		return emitJSON(f)
	}
	b, err := f.Marshal()
	if err != nil {
...
func snapshotSettings(opts provision.Options) skrogfile.File {
	f := skrogfile.File{}
...
func applyProfile(ctx context.Context, f skrogfile.File, opts provision.Options) error {
	sd := opts.StateDir
