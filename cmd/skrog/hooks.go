...
func runHook(stateDir, event, path string, log *slog.Logger) {
	...
	// The hook learns which event fired and where Skrog keeps its state, so one
	// script can serve several events and reach `skrog` if it wants to.
	cmd.Env = append(os.Environ(),
		"SKROG_EVENT="+event,
		"SKROG_STATE_DIR="+stateDir,
	)
