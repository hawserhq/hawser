...
// runLogs is `skrog logs` (#146): the supervisor, dockerd, or audit log, with
...
func runLogs(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: skrog logs [--source supervisor|dockerd|audit] [-n <lines>] [--follow] [--json]
...
`, exitError, exitUsage, exitNotFound)
	...
	default:
		fmt.Fprintf(os.Stderr, "skrog: unknown --source %q (supervisor, dockerd, audit)\n", *source)
		return exitUsage
	}
}
...
func tailFile(ctx context.Context, path string, n int, follow bool, out lineEmitter) int {
	...
	case errors.Is(err, os.ErrNotExist):
		fmt.Fprintf(os.Stderr, "skrog: no %s log yet at %s\n", out.source, path)
		if !follow {
			return exitOK
		}
		offset = 0
...
func tailDockerd(ctx context.Context, opts provision.Options, n int, follow bool, out lineEmitter) int {
	...
	if err := cmd.Start(); err != nil {
		fmt.Fprintf(os.Stderr, "skrog: reading dockerd log: %v\n", err)
		return exitError
	}
