...
func TestConsoleHandlerCarriesWithAttrs(t *testing.T) {
	...
	log := slog.New(newConsoleHandler(&buf, slog.LevelInfo)).With("distro", "skrog-engine")
	log.Info("engine recovered")
	out := buf.String()
	if !strings.Contains(out, "engine recovered") || !strings.Contains(out, "distro=skrog-engine") {
...
func TestConsoleHandlerFixOnItsOwnLine(t *testing.T) {
	var buf bytes.Buffer
	slog.New(newConsoleHandler(&buf, slog.LevelWarn)).Warn("cannot start", "fix", "run skrog install")
	out := buf.String()
	if !strings.Contains(out, "warning: cannot start") || !strings.Contains(out, "\n    fix: run skrog install") {
