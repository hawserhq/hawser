...
// runReset is `skrog reset --to <snapshot>` (#142): the runner's clean slate.
...
func runReset(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: skrog reset --to <snapshot> [--json]
...
`, exitError, exitUsage, exitNotFound)
	...
	if *to == "" {
		fmt.Fprintln(os.Stderr, "skrog: reset needs --to <snapshot>")
		return exitUsage
	}
