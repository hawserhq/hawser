...
func runCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: skrog cli <install|status|uninstall> [flags]")
		return exitUsage
	}
	switch args[0] {
	case "install":
	...
	default:
		fmt.Fprintf(os.Stderr, "skrog cli: unknown subcommand %q (install|status|uninstall)\n", args[0])
		return exitUsage
	}
}

// cliBinDir is where the bundled docker.exe and credential helper live: the
// directory Skrog puts on PATH.
...
func runCLIInstall(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `usage: skrog cli install [--no-path]
...
`, exitError, exitUsage)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "skrog: installing the docker CLI bundle: %v\n", err)
		return exitError
	}
	...
	if !*noPath {
		...
		switch {
		case err != nil:
			fmt.Fprintf(os.Stderr, "\nskrog: could not update your PATH (%v).\n"+
...
func runCLIStatus(args []string) int {
	...
	if anyMissing {
		fmt.Printf("\nRun `skrog cli install` to install the missing tools.\n")
		return exitNotFound
	}
	return exitOK
}
...
func runCLIUninstall(args []string) int {
	...
	if removed, err := dockercli.RemoveFromUserPath(binDir); err != nil {
		fmt.Fprintf(os.Stderr, "skrog: could not update PATH: %v\n", err)
	} else if removed {
		fmt.Printf("Removed from user PATH: %s\n", binDir)
	}
	if err := os.RemoveAll(binDir); err != nil {
		fmt.Fprintf(os.Stderr, "skrog: removing %s: %v\n", binDir, err)
		return exitError
	}
