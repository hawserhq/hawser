...
//
// SKROG_NO_VSOCK=1 pins the socat path, as the support lever for a machine
...
func engineDialer(distro, socketPath, stateDir string, log *slog.Logger) pipeproxy.Dialer {
	...
	if os.Getenv("SKROG_NO_VSOCK") == "1" {
		log.Info("vsock transport disabled by SKROG_NO_VSOCK")
		return socat
	}
	secret := ""
	if stateDir != "" {
