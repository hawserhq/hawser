# Hawser's one-line installer (#77).
#
...
# one-liner should do without being asked. `hawser install` is the next step,
# and this script says so when it finishes.
#
# Options, because `irm | iex` cannot pass arguments, come from the environment:
#
#   $env:HAWSER_VERSION = '0.3.0'          # default: the newest release
#   $env:HAWSER_DIR     = 'C:\tools\hawser' # default: %LOCALAPPDATA%\Programs\hawser
#   $env:HAWSER_NO_PATH = '1'               # do not touch PATH
#
# Invoked as a file or a script block, the parameters below work normally:
#
#   & ([scriptblock]::Create((irm https://hawserhq.github.io/hawser/install.ps1))) -Version 0.3.0
#
# A CI runner should use the action instead: https://github.com/hawserhq/setup-hawser
...
[CmdletBinding()]
param(
    [string]$Version = $env:HAWSER_VERSION,
    [string]$Dir = $env:HAWSER_DIR,
    [switch]$NoPath = [bool]$env:HAWSER_NO_PATH
)
...
if (-not $Version) {
    ...
    $rels = Invoke-RestMethod -Headers @{ 'User-Agent' = 'hawser-install' } `
...
Say ""
Say "Hawser $Version ($arch)"

# --- 2. download and verify --------------------------------------------------

$work = Join-Path ([IO.Path]::GetTempPath()) "hawser-install-$Version"
...
try {
    ...
    if (-not $Dir) { $Dir = Join-Path $env:LOCALAPPDATA 'Programs\hawser' }
    # Extracting over a running hawser.exe fails with a file lock, which is a
    # better error than a half-replaced install, but a useless one on its own.
    $running = Get-Process -Name hawser, hawserw, hawsertray -ErrorAction SilentlyContinue |
        Where-Object { $_.Path -and $_.Path.StartsWith($Dir, [StringComparison]::OrdinalIgnoreCase) }
    if ($running) {
        throw ("Hawser is running from $Dir (" + (($running.Name | Sort-Object -Unique) -join ', ') + "). " +
            "Stop it first: hawser stop; then quit the tray if it is open.")
    }

    New-Item -ItemType Directory -Force $Dir | Out-Null
    Expand-Archive $zipPath -DestinationPath $Dir -Force
    $exe = Join-Path $Dir 'hawser.exe'
    if (-not (Test-Path $exe)) { throw "hawser.exe not found after extracting $zip" }
...
# `hawser version` exits 3 when no engine is installed, which is the expected
...
Say ""
Say "Installed $(if ($reported) { $reported } else { "hawser $Version" })"
Say "  $exe"
if ($added) {
    Say "  added to your PATH (open a new terminal to pick it up)"
} elseif ($NoPath) {
    Say "  PATH not modified (HAWSER_NO_PATH)"
} else {
...
Say ""
Say "Next:"
Say "  hawser install      provision the engine (downloads a verified rootfs, ~2 min)"
Say "  hawser doctor       check this machine is ready first"
Say ""
Say "Binaries are not signed yet, so Windows SmartScreen may warn on first run."
Say "Docs: https://hawserhq.github.io/hawser/"
Say ""
