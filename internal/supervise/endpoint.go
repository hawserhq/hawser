...
// WriteEndpoint records the binding atomically, so a reader sees a whole
// record or none. Failure is the caller's to log and carry on with: a status
// field is not worth taking the bridge down for.
func WriteEndpoint(stateDir string, e Endpoint) error {
	...
	if err := os.Rename(tmp, endpointPath(stateDir)); err != nil {
