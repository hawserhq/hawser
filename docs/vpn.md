...
`skrog doctor` recognizes the common VPN clients from your active network
...
not just Skrog's — Skrog does not edit it for you. Add it yourself and restart
WSL:
...
restart WSL for the change to take effect, then `skrog start`.

If the engine still stalls on large pulls, the remaining lever is the engine's
own MTU. Unlike `~/.wslconfig`, that is Skrog's distro, so there is a command
for it — use the value `doctor` reports:

```
skrog config set engine.mtu 1400
```
...
```
skrog config set network.import-host-cas on
skrog restart
```

## Published ports under mirrored networking

Mirrored networking changes how `docker run -p` has to be plumbed, and Skrog
...
Skrog applies the default to installs that predate it, on the next engine
start, and never overrides a value you set yourself:

```
skrog config get engine.userland-proxy     # false
skrog config set engine.userland-proxy on  # your choice wins from then on
```
...
`skrog doctor` names the VPN and shows the exact settings rather than applying
...
change only Skrog's distro, so those are commands rather than advice. The line
is ownership — Skrog configures what it owns and tells you about the rest.
