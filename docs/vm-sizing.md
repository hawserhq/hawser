...
So Hawser splits it in two. You record what you want in Hawser's own settings;
...
| Hawser setting | `.wslconfig` key | what it does |
| --- | --- | --- |
...
fails at `hawser config set` rather than silently sizing the VM as something
else. `memory=4` in `.wslconfig` means *four bytes*; `hawser config set
wsl.memory 4` refuses and tells you to write `4GB`.

Networking keys (`networkingMode`, `dnsTunneling`, `autoProxy`) are **not**
managed here. `hawser doctor` recommends those for VPNs ([vpn.md](vpn.md)) and
...
```
$ hawser wsl-config apply --yes
wrote 3 change(s) to C:\Users\me\.wslconfig
$ hawser wsl-config apply --yes
C:\Users\me\.wslconfig already matches Hawser's settings; nothing to do
```

**Everything else in the file survives** — other sections, keys Hawser does not
manage, comments (including trailing ones on a line it edits), blank lines and
your ordering. The file belongs to you; Hawser edits the lines it was asked to
and nothing more.

`hawser wsl-config show` reports the effective sizing and anything pending, and
`--json` gives `effective`, `desired`, `pending` and `applied` for a converge
script.

## It takes effect on the next VM start, and Hawser will not force that
...
containers included — so Hawser tells you rather than doing it:

```
  the new sizing takes effect when the WSL VM next starts.
  `hawser stop` does not do that (it stops only Hawser's distro); a reboot,
  or `wsl --shutdown` when you are sure nothing else is running, does.
```

`hawser stop` is not enough on purpose: it stops Hawser's distro, and the
utility VM stays up for as long as any distro is running.

## What doctor says about it

`hawser doctor` always reports the effective sizing, and judges it in exactly
two cases:

- **Recorded but not applied.** You ran `hawser config set wsl.memory 4GB` and
  never applied it, so the cap you believe is in force is not. Warn, with
  `hawser wsl-config apply` as the remedy — this is the mistake the two-step
...
- [housekeeping.md](housekeeping.md) — `hawser prune` and `hawser compact` for
